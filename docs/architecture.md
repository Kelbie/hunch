...
**Fetch the packs a config names at review time:** simpler to write, but it makes a review depend on what a third-party repository says today, needs network from the App's worker, and leaves no record of which rules ran. `hunch install` copies a configured pack into `hunch.lock` instead, pinned to the commit it came from, and a review reads only the lock. A pack is already written as rules, so it is copied verbatim and no model is involved: the compiler exists for prose, and a repository whose policy is only packs installs with no agent, no key and no model cost. Pack rules keep their author's level, threshold and file scope; the config can re-level, disable or replace one by id, and its own `budget` still governs the run. A selection pattern that matches no rule fails the install rather than quietly reviewing less.
