...
Run `npx @kelbie/hunch init`, optionally name packs or install skills, run `npx @kelbie/hunch install`, review and commit those files to the default branch. Then open a PR. Policy is always read from the immutable base SHA, so a PR cannot rewrite its own rules. A PR that first introduces Hunch is skipped until the config is on its base branch.
