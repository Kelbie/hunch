...
try {
  ...
  if (!run(process.execPath, [bin, '--version']).includes('0.3.2')) throw new Error('Version command failed');
  ...
  writeFileSync(join(rust, 'package.json'), '{"devDependencies":{"@kelbie/hunch":"0.3.2"}}');
  ...
  run(process.execPath, [bin, 'init', '--general', '--github', '--cwd', generalNpm]);
  if (!existsSync(join(generalNpm, 'hunch.toml'))) throw new Error('--general ignored in npm consumer');
  const ts = join(temp, 'typescript'); mkdirSync(ts); run('git', ['init', '--quiet'], ts);
  run(process.execPath, [bin, 'init', '--ts', '--github', '--cwd', ts]);
